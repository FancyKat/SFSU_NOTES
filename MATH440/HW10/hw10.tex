...
\subsection*{Question}
% The question involves determining the PDF of W given X, where W is the majority function of x, y, and z.
...
% Calculating the probability that the majority is 1 when X = 1
\[
P(W = 1 | X = 1) = P(1, 1, 1) + P(1, 1, 2) + P(1, 2, 1) + P(2, 1, 1)
\]
% Using the joint PDF for each specific case
\[
...
% Calculating the probability that the majority is 2 when X = 1
\[
...
% Repeating the structure for when X = 2
\subsubsection*{Similar Calculations as Above}
% Use comments to guide the structure for calculating probabilities when X = 2
